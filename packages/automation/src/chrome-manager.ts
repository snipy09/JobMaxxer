...
export function findChromeExecutable(): string | undefined {
  if (process.platform === 'win32') {
    const candidatePaths: (string | undefined)[] = [
      // Chrome for Testing local downloads
      path.join(process.env.LOCALAPPDATA || '', 'JobMaxxer', 'chrome-for-testing', 'chrome-win64', 'chrome.exe'),
      path.join(process.env.LOCALAPPDATA || '', 'JobMaxxer', 'chrome-for-testing', 'chrome.exe'),
      path.join(process.env.APPDATA || '', 'JobMaxxer', 'chrome-for-testing', 'chrome.exe'),
