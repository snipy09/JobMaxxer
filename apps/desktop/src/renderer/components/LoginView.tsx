...
import {
  ...
  ArrowRight, CheckCircle2, X, RefreshCw
} from 'lucide-react';
      ...
      if (authMode === 'signup') {
        if (api.authSignup) {
          ...
          if (res.success && res.user) {
            onLog(`[Auth] Account created for ${res.user.email} ✓`);
            onLoginSuccess(res.user);
          } else {
            setErrorMsg(res.error || 'Sign up failed. Please check your details.');
          }
        } else {
        ...
        if (res.success && res.user) {
          onLog(`[Auth] Welcome back, ${res.user.email} ✓`);
          onLoginSuccess(res.user);
        } else {
      ...
      <div className="w-full max-w-sm space-y-6 relative z-10 animate-fade-up">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">
            {authMode === 'signin' ? 'Welcome back to Nomadic' : 'Create your Nomadic Account'}
          </h1>
        ...
        <div className="bg-white dark:bg-[#121215] border border-slate-200/80 dark:border-zinc-800 rounded-3xl p-6 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-2 p-1 bg-slate-100 dark:bg-zinc-900 rounded-xl text-xs font-semibold">
            <button
              type="button"
          ...
          ══════════════════════════════════════════════════════════════ */}
          {googleLoading ? (
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900/90 border border-slate-200 dark:border-zinc-800 space-y-3.5 animate-fade-up">
              {googleSuccess ? (
                ...
                <>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-mono">Listening on port 42813 ({googleWaitSeconds}s)</p>
                      </div>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="p-3 bg-white dark:bg-zinc-800/80 rounded-xl border border-slate-200/80 dark:border-zinc-700/80 space-y-2 text-[11px]">
                    <div className="flex items-center gap-2 text-slate-700 dark:text-zinc-300">
                      ...
                      <span>Secure loopback server active</span>
                    </div>
            ...
            <button
              ...
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full py-3 px-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800 rounded-2xl text-xs font-bold text-slate-800 dark:text-zinc-100 transition-all flex items-center justify-center gap-3 shadow-xs hover:shadow active:scale-98 disabled:opacity-50"
            >
              ...
              <span>{authMode === 'signin' ? 'Continue with Google' : 'Sign up with Google (1-Click)'}</span>
            </button>
          )}
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="bg-white dark:bg-[#121215] px-3 text-[10px] text-slate-400 font-mono uppercase tracking-wider">or with email</span>
            <div className="border-t border-slate-100 dark:border-zinc-800 w-full" />
          </div>
          ...
          <form onSubmit={handleManualSubmit} className="space-y-3.5">
            {authMode === 'signup' && (
              <div className="space-y-1 text-xs animate-fade-up">
                <label className="font-semibold text-slate-700 dark:text-zinc-300">Full Name</label>
                <input
                  type="text"
                  value={fullName}
                  onChange={(e) => setFullName(e.target.value)}
                  placeholder="Alex Vance"
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-zinc-100 outline-none focus:border-slate-400 dark:focus:border-zinc-500 text-xs transition-colors"
                />
              </div>
            )}
            ...
            <div className="space-y-1 text-xs">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 text-slate-900 dark:text-zinc-100 outline-none focus:border-slate-400 dark:focus:border-zinc-500 text-xs transition-colors pr-10"
                />
                <button
                  type="button"
            ...
            {errorMsg && (
              <div className="text-[11px] text-rose-600 dark:text-rose-400 font-semibold flex items-center gap-1.5 p-2.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/60 animate-fade-up">
            ...
            <button
              ...
              className="w-full py-3 bg-slate-950 hover:bg-slate-800 text-white dark:bg-white dark:hover:bg-slate-100 dark:text-slate-950 rounded-2xl text-xs font-bold transition-all flex items-center justify-center gap-2 shadow-sm hover:shadow active:scale-98 disabled:opacity-50 mt-2"
            >
              {loading ? (
                <>
                  <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  <span>{authMode === 'signin' ? 'Authenticating credentials...' : 'Creating your account...'}</span>
                </>
              ) : (
                <>
                  <span>{authMode === 'signin' ? 'Sign In to Workspace' : 'Create Nomadic Account'}</span>
